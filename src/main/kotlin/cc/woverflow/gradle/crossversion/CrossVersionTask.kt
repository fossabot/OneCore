...
open class CrossVersionTask : DefaultTask() {
    companion object {
        @JvmStatic
        ...
        private val LOGGER = LoggerFactory.getLogger(CrossVersionTask::class.java)
    }
    ...
    fun crossversion() {
        ...
        if (classpath != null && (mapping != null || sourceMappings != null && destinationMappings != null)) {
            ...
            source.flatMap { base -> project.fileTree(base).map { Pair(base, it) } }.forEach { (base, file) ->
                if (file.name.endsWith(".java") || file.name.endsWith(".kt")) {
                    ...
                    if (kws != null) {
                        processedSources[relPath.toString()] = CommentCrossVersion(vars.get()).convertSource(
                                kws.value,
                                lines,
        ...
        val commentCrossVersion = CommentCrossVersion(vars.get())
        source.flatMap { base -> project.fileTree(base).map { Pair(base, it) } }.forEach { (base, file) ->
            val relPath = base.toPath().relativize(file.toPath())
            ...
            if (kws != null) {
                ...
                commentCrossVersion.convertFile(kws.value, file, outFile, javaTransform)
            } else {
                project.copy {
                    from(file)
        ...
        if (commentCrossVersion.fail) {
            throw GradleException("Failed to remap sources. See errors above for details.")
        }
    }
}

class CommentCrossVersion(private val vars: Map<String, Int>) {
    companion object {
