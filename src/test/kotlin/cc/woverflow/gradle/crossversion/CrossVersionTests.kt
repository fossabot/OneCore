...
class CrossVersionTests : FunSpec({
    val vars = mapOf(
            "zero" to 0,
            "one" to 1,
            "two" to 2,
    ...
    with(CommentCrossVersion(vars)) {
        ...
        context("convertSource") {
            fun String.convert() = convertSource(
                    CrossVersionTask.DEFAULT_KEYWORDS,
                    lines(),
            ...
            test("throws on unexpected endif") {
                shouldThrow<CommentCrossVersion.ParserException> { "//#endif".convert() }
            }
            test("throws on unexpected else") {
                shouldThrow<CommentCrossVersion.ParserException> { "//#else".convert() }
            }
            test("throws on unexpected elseif") {
                shouldThrow<CommentCrossVersion.ParserException> { "//#elseif".convert() }
            }
            test("throws on elseif after else") {
                shouldThrow<CommentCrossVersion.ParserException> { """
                    //#if t
                    //#else
                    //#elseif t
                    //#endif
            ...
            test("throws on missing endif") {
                shouldThrow<CommentCrossVersion.ParserException> { "//#if t".convert() }
                shouldThrow<CommentCrossVersion.ParserException> { "//#if t\n//#if t\n//#endif".convert() }
            }
            test("if t .. endif") {
                """
                    //#if t
